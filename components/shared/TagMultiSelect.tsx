...
export function TagMultiSelect({ availableTags, value, onChange }: TagMultiSelectProps) {
    ...
    const selectedTags = value.map(id => availableTags.find(tag => tag.id === id)).filter(Boolean) as {id: string, name: string}[];
                    ...
                    <Command>
                        ...
                        <CommandGroup>
                            {availableTags.map((tag) => (
                                ...
                                >
                                    <Check
                                        className={cn('mr-2 h-4 w-4', value.includes(tag.id) ? 'opacity-100' : 'opacity-0')}
                                    />
                                    {tag.name}
                                </CommandItem>
                            ))}
